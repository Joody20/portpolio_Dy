...
export const project7 = [
  {
    ...
    features: "▷ 주요 특징 및 기능",
    feat1:
      "카카오테크 부트캠프 구성원들이 경험과 일상을 자유롭게 공유할 수 있는 커뮤니티 서비스 구현.",
    feat2:
      "Vanilla JS 기반 구현에서 출발해 Virtual DOM 기반 SPA 구조로 고도화하며 상태 관리와 렌더링 구조 개선.",
    feat3:
      "이후 React로 마이그레이션하며 컴포넌트 구조와 재사용성을 강화하고 프론트엔드 아키텍처를 단계적으로 개선.",
    feat4:
      "한 프로젝트 안에서 Vanilla JS → Custom Virtual DOM SPA → React로 이어지는 단계적 전환 과정을 직접 설계하고 구현.",
    ...
    journey_sections: [
      ...
      },
      {
        ...
        points: [
          "Axios 인터셉터로 401 응답 시 Access Token 자동 재발급 및 대기 요청 순차 재시도 로직 구현.",
          "JWT 자동 재발급 기반 인증 흐름으로 로그인 세션이 끊기지 않도록 안정화.",
          "IntersectionObserver 기반 커서 무한스크롤과 debounce 검색 최적화 적용.",
          "좋아요 기능에는 낙관적 업데이트와 실패 시 롤백 처리를 적용해 즉각적인 반응성 확보.",
        ],
      },
    ],
    ...
    troubleshooting_items: [
      ...
      },
      {
        ...
        problem:
          "검색창 입력 시 한 글자마다 API가 호출되어 서버 부하가 커졌고, 제목과 내용 검색어 입력마다 Virtual DOM 전체 diff 및 리렌더링이 발생해 타이핑 도중 지연이 발생했습니다. 또한 검색 버튼과 엔터 키를 연타할 경우 중복 결과가 누적되거나 UI가 튀는 현상, 게시글 작성 버튼 더블 클릭 시 동일 게시글이 중복 생성되는 문제도 있었습니다.",
        solution:
        ...
        result:
          "불필요한 API 요청 수와 리렌더링 빈도를 줄여 입력 반응성을 개선했고, 버튼 연타나 빠른 상호작용에서도 중복 요청 없이 안정적인 UI 흐름을 유지하도록 했습니다.",
        images: [
      ...
      },
      {
        ...
        cause:
          "renderPage()가 Virtual DOM을 생성해 반영하는 과정과 실제 DOM 접근 시점이 어긋나면서, initPostListPage() 실행 시 아직 #post_list가 생성되지 않아 참조 코드가 중단되는 문제가 있었습니다. 여기에 SPA 특성상 이전 페이지의 prevNode와 DOM 상태가 메모리에 남아 재진입 시 VDOM 엔진이 동일 구조로 잘못 안자해 업데이트를 건너뛰었고 IntersectionObserver와 이벤트 리스너 clean up이 부족해 재바인딩 충돌까지 이어졌습니다.",
        solution:
